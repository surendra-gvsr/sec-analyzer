...
function TriggerButton({ label, action, onDone }: TriggerButtonProps) {
  ...
  const run = async () => {
    ...
    try {
      ...
      if (polls >= MAX_POLLS) {
        setStatus({ job_id: current.job_id, stage: 'error', progress: current.progress, message: 'Timed out waiting for job to complete', errors: ['Timeout after 5 minutes'] })
        return
      }
      onDone()
    } catch (e) {
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-4">
      <div className="flex items-center justify-between mb-3">
        ...
        <Button onClick={run} disabled={loading} size="sm" variant="outline" className="min-h-11 hover:bg-muted">
          {loading ? 'Running…' : 'Run'}
        </Button>
      </div>
      {status && (
