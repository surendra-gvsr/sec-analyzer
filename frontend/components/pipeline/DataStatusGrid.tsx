      ...
      {tickers.map(ticker => (
        <div key={ticker} className="rounded-xl border border-border bg-card p-4">
                ...
                <div key={yr} className="flex items-center gap-1.5 text-xs text-muted-foreground">
                  <span
                    className={cn(
                      ...
                      parsed ? 'bg-chart-2' : downloaded ? 'bg-chart-3' : 'bg-muted-foreground/30',
                    )}
                  />
                  {yr}
                </div>
          ...
          <div className="mt-2 flex gap-3 text-[10px] text-muted-foreground">
            <span className="flex items-center gap-1"><span className="h-1.5 w-1.5 rounded-full bg-chart-2 inline-block" /> Parsed</span>
            <span className="flex items-center gap-1"><span className="h-1.5 w-1.5 rounded-full bg-chart-3 inline-block" /> Downloaded</span>
            <span className="flex items-center gap-1"><span className="h-1.5 w-1.5 rounded-full bg-muted-foreground/30 inline-block" /> Missing</span>
          </div>
        </div>
      ))}
    </div>
